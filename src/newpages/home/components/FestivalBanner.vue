    ...
    <div class="fb-stage">
      ...
      <div v-if="loading || !slides.length" class="fb-empty">
        <span class="bodyMedium14px">
          {{ loading ? '축제 정보를 불러오는 중입니다.' : '표시할 축제가 없습니다.' }}
        </span>
      </div>
    </div>
...
async function load() {
  loading.value = true
  try {
    const { data } = await axios.get(props.endpoint, { withCredentials: true })
    applyData(data)
...
const indexLabel = computed(() => (total.value ? `${cur.value + 1}/${total.value}` : ''))

function onClick() {
  if (!props.linkToDetail) return
  const s = slides.value[cur.value]
  if (s?.id) router.push({ name: 'TourplaceDetailV2', params: { id: String(s.id) } })
}
function onKey(e) {
...
.fb-overlay {
  position: absolute;
  inset: 0;
  background: linear-gradient(0deg, rgba(0,0,0,.55), rgba(0,0,0,0) 65%);
  display: flex;
