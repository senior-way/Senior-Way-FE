  ...
  <div class="page">
    ...
    <section v-else-if="currentKey === 'duration' " class="sec sec-duration">
      ...
      <form class="cta-wrap" @submit.prevent="finish">
        <SubmitButton label="추천 일정 생성하기" variant="primary" :disabled="sel.duration === null || posting" />
      </form>
      <p class="note bodyMedium16px">현재 <a href="#">선호도 조사</a> 진행중입니다.</p>
    </section>
  </div>
...
const canProceed = computed(() => ({
  ...
  pets:       !!sel.value.pets,
  device:     !!(typeof sel.value.device === 'object' ? sel.value.device?.key : sel.value.device)
}))

function indexOfKey (key) { return steps.findIndex(s => s.key === key) }

function goNext () {
  if (currentKey.value === 'device') { currentKey.value = 'duration'; return }
...
const moveModel = computed({
  get() { return Array.isArray(sel.value.move) ? sel.value.move : (sel.value.move ? [sel.value.move] : []) },
  set(next) { sel.value.move = Array.isArray(next) ? next : (next ? [next] : []) }
})

const deviceModel = computed({
  get() { return typeof sel.value.device === 'object' && sel.value.device !== null ? (sel.value.device.key ?? '') : (sel.value.device ?? '') },
  set(next) { sel.value.device = typeof next === 'object' && next !== null ? (next.key ?? '') : (next ?? '') }
})
...
const durationModel = computed({
  get() { return sel.value.duration === null ? [] : [sel.value.duration] },
  set(arr) { sel.value.duration = Array.isArray(arr) && arr.length ? Number(arr[arr.length - 1]) : null }
})
...
function finish () {
  ...
  const payload = {
    exp: sel.value.exp,
    move: moveModel.value,
    wheelchair: String(sel.value.wheelchair).includes('사용합니다'),
    pets: String(sel.value.pets).includes('동반합니다'),
    deviceLevel: deviceModel.value,
    nights,
    days: nights + 1
  }

  // 백엔드 연동 사용
  // axios.post('/api/schedules/recommend', payload, { withCredentials: true })
  //   .then(res => {
  //     const rid = res?.data?.id || res?.data?.scheduleId
  //     router.push({ name: 'ScheduleTimelineV2', query: rid ? { rid: String(rid) } : {} })
  //   })
  //   .catch(() => {
  ...
  const spotPool = [
    { id: 501, name: '광안리 해수욕장',    image: 'https://picsum.photos/id/1011/400/250' },
    { id: 502, name: '송도 해상 케이블카', image: 'https://picsum.photos/id/1012/400/250' },
    { id: 503, name: '부산시립미술관',      image: 'https://picsum.photos/id/1014/400/250' },
    { id: 504, name: '부산타워',            image: 'https://picsum.photos/id/1020/400/250' },
    { id: 505, name: '해운대 해수욕장',     image: 'https://picsum.photos/id/1016/400/250' }
  ]
  ...
  const stayMin = 90
  const gapMin = 20
  const totalStops = Math.min(spotPool.length, 5)
  ...
  for (let i = 0; i < totalStops; i++) {
    ...
    cursor = addMin(cursor, stayMin + gapMin)
  }

  const end = addMin(start, (stayMin + gapMin) * totalStops)
  const title = nights === 0 ? '당일치기 추천 일정' : `${nights}박 ${nights+1}일 추천 일정`

  localStorage.setItem(`mockSchedule:${rid}`, JSON.stringify({
    title,
